import React from 'react'
import "./Home.css";
import { Link } from 'react-router-dom';
const Home = () => {
    const image=require("../images/smart-boy.avif");
  return (
    <div className="Home-Page bg-dark text-white container-fluid">
<div className="row container">
<div
className="col-lg-6 d-flex justify-content-center align-items-center align-items-lg-start flex-column"
style={{ height: "91.5vh" }}
>
<h2 style={{ fontSize: "80px" }}>BOOK STORE</h2>
<h3 style={{ fontSize: "50px" }}>FOR YOU</h3>
<p className="mb-0" style={{color:"silver"}}>Checkout The Books</p>
<Link to="/books" className='viewBook' my-3 style={{ textDecoration: "underline" }}>View Books</Link>
<p className="mb-0" style={{ color: "silver" }}>Contact to Buy:
  <a href="tel:+1234567890" style={{ color: "silver", textDecoration: "none", textAlign: "center", fontSize: "18px", color: "white" }}>
  6329458270
  </a>
</p>

</div>
<div
className="col-lg-6 d-flex justify-content-center align-items-center flex-column"
style={{ height: "91.5vh" }}>
<img className="img-fluid homing" src={image} alt="/"/>
</div>
</div>
</div>
  );
};

export default Home